def rf_grid_comorbilidad(file, p):
    import h2o
    ...
    # train_df = train_df.rename(columns={'Lupus eritematoso sistémico': 'Lupus eritematoso sistemico'})
    # train_df = train_df.rename(columns={'Síndrome de Sjögren': 'Sindrome de Sjogren'})
    #
    # valid_df = valid_df.rename(columns={'Lupus eritematoso sistémico': 'Lupus eritematoso sistemico'})
    # valid_df = valid_df.rename(columns={'Síndrome de Sjögren': 'Sindrome de Sjogren'})
    #
    # test_df = test_df.rename(columns={'Lupus eritematoso sistémico': 'Lupus eritematoso sistemico'})
    # test_df = test_df.rename(columns={'Síndrome de Sjögren': 'Sindrome de Sjogren'})

    h2o.init(nthreads=-1, max_mem_size=8)
    h2o.connect()
    if p == 1:
        y = 'Ventilacion'
        x = list(train_df.columns[2:35])
    elif p == 2:
        y = 'Mortalidad'
        x = list(train_df.columns[2:36])
